...
x = xopt(result_B);
% x = xopt(1:n);
result_N = setdiff(1:m+n,result_B);
y = xopt(result_N);
...
Nx = intersect(1:n,result_N,'stable');
Ny = setdiff(result_N,Nx,'stable');
...
while(length(Bx) < m)
    ...
    for temp_j = Nx
        ...
        I_Byn = I(:,By-n);
        A_Bx = A(:,Bx);
        concat_Matrix = [I_Byn A_Bx];
        A_temp_j = A(:,temp_j);
        w = concat_Matrix\A_temp_j;
        indices_i = find(w(1:i) ~= 0);
%         indices_of_w_unequal_zero = find(w ~= 0);
%        indices_leq_i = indices_of_w_unequal_zero(indices_of_w_unequal_zero <= m - length(Bx)); 
%         if(~isempty(indices_leq_i))
        if(~isempty(indices_i))
           i = indices_i(1); 
           j = temp_j;
           break; 
        end
...
%     A_Bx = A(:,Bx);
%     concat_matrix = [I_Byn A_Bx];
%     all_w = concat_matrix \ A(Nx);


    if(~isempty(j))
            Bx = union(Bx,j,'stable');
            By = setdiff(By,By(i),'stable');
            Nx = setdiff(Nx,j,'stable');
